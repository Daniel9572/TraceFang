...
- 缺失范围只能由独立回补 POST 触发。它使用同一绑定源的历史能力，先检查 `realtime_candle_cache_ranges`，再下载、校验并写入 `candles`；GET 本身永不访问上游。金十可变历史文件按 manifest 条数和结束时间建立下载版本键，本地文件缓存也必须覆盖当前 manifest 声明区间。成功零行记录覆盖，失败不记录。
- 图表把左边界交互转换为一个可持续的历史需求；需求在本地分页锁占用期间保留，解锁后重新评估，直到视窗获得足够数据、上游失败或连续空窗口达到安全边界。完整已加载序列仍无法覆盖视窗时可以自动建立需求；普通程序性重绘在视窗已有足够数据时不访问上游。回补完成后按精确前插点数平移逻辑范围，保持用户正在查看的位置。
- 同一交易时段内的可见 Bar 缺口可以触发一次精确同源复核；它只绕过该子区间的粗粒度覆盖缓存，不跨来源、不把计划休市当缺口，也不改写原始证据身份。
