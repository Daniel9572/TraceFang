...
过去 K 线回补使用同一个 `jin10_local` 会话：订阅 `time_type=1` 的分钟 K 线，协议 `10006` 以目标缺失窗口的结束时间作为边界，直接请求该边界之前的历史文件目录，再从官方文件服务下载 GZip 数据；不再从最新目录反复向前翻页。页面 K 线 GET 始终只读本地；仅在 PostgreSQL 没有该范围的完成覆盖时，后台回补命令才访问上游。文件校验、下载和数据库写入均在报价热路径之外执行；相同范围的并发请求合并，成功但零行的休市范围也缓存。
