...
> 冻结于 2026-08-08。本文以下内容只保存旧方案、离线数据资产和校验结果，不是当前运行规则。当前权威语义见 [data-semantics.md](data-semantics.md)：历史数据是时间范围；一个合约的当前与过去数据必须全部来自其绑定的同一个实时数据源。运行时不得使用本文的跨源准入、可信视图、标准分段或补缺流程。

当前冻结状态：

- `history` schema、导入器、校验器和 HistData 候选不接入页面或 API；
- `standard_candles`、`candle_validation_results` 与 `canonical_segments` 不参与运行时读取或写入；
- 不执行跨实时数据源确认、准入、拼接、补缺或静默替换；
- 下列命令与流程仅作研究记录，解冻前不得用于生产数据。
...
- 这里的“渠道”是历史证据的内部来源身份，不是用户可选择的实时数据源。该旧方案的跨渠道治理当前不进入运行时。
