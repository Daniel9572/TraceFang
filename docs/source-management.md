...
1. `jin10_local`（金十桌面会话行情）：使用本机金十客户端会话鉴权的结构化 WebSocket 报价，并从同一报价流生成分钟 K 线；
2. `jin10_web`（金十官网高速行情）：金十官网公开、无需登录口令的结构化价格变化推送；
3. `jin10_mcp`：官方结构化报价、K 线、目录、资讯与日历。
...
- `GET /api/quotes/{code}?source=jin10_mcp|jin10_local|jin10_web`：读取报价；
...
- 网页极速报价使用独立公开长连接和变化驱动档；2026-08-06 实测黄金 28 秒 71 帧，常见间隔约 0.1～0.4 秒，不消耗 MCP 工具额度；
...
页面用全局历史的最近一分钟 K 线作为基线，并把当前合约实时源的新报价合并进当前柱。同一源时间秒内的多个事件以本地接收时间排序，全部参与当前柱高低收计算；当前价始终立即显示最新帧，不为逐个播放旧帧而制造延迟。收盘倒计时完全在浏览器本地计算，不消耗 MCP 调用额度。
