...
> 冻结记录（2026-08-08 复核）：本项目暂时不再使用金十 MCP。以下内容只保留为历史协议资料；应用运行时不读取 MCP 环境配置、不创建客户端、不握手、不执行工具调用，也不把 MCP 注册为实时数据源或内部通道。当前不再生成全局标准历史，既有 MCP 记录也不会进入任何绑定源的当前或过去数据缓存。若未来解冻，`get_quote + get_kline` 必须作为同一个完整实时数据源的两项能力重新评审。
