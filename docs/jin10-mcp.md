...
> 冻结记录（2026-08-07）：本项目暂时不再使用金十 MCP。以下内容只保留为历史协议资料；当前 `JIN10_MCP_ENABLED=false`，服务启动不创建客户端、不握手、不执行工具调用，MCP 不作为实时、历史、补缺或校对数据源。任何含 MCP 证据的标准历史都会被剔除。
...
历史实现会按“进程环境变量优先于 `.env`”读取配置；冻结期间运行时在读取 Token 和构建 MCP 客户端之前即停止该分支。Token 与额度字段保留仅为归档，不表示当前可启用。
