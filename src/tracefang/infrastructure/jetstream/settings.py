...
class JetStreamSettings:
    ...
    def from_env(cls) -> JetStreamSettings | None:
        url = os.environ.get("TRACEFANG_NATS_URL", "").strip()
        if not url:
            return None
        return cls(
            url=url,
            stream_name=os.environ.get("TRACEFANG_NATS_STREAM", "MARKET_RAW_FRAMES").strip(),
            subject_prefix=os.environ.get(
                "TRACEFANG_NATS_SUBJECT_PREFIX", "market.raw"
            ).strip(),
            connect_timeout_seconds=float(
                os.environ.get("TRACEFANG_NATS_CONNECT_TIMEOUT_SECONDS", "2")
            ),
            publish_timeout_seconds=float(
                os.environ.get("TRACEFANG_NATS_PUBLISH_TIMEOUT_SECONDS", "2")
            ),
            max_age_seconds=float(os.environ.get("TRACEFANG_NATS_MAX_AGE_SECONDS", "604800")),
            max_bytes=int(os.environ.get("TRACEFANG_NATS_MAX_BYTES", "10737418240")),
            max_frame_bytes=int(
                os.environ.get("TRACEFANG_NATS_MAX_FRAME_BYTES", "33554432")
            ),
        )
