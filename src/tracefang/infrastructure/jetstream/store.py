    ...
    async def capture(self, envelope: FrameEnvelope) -> int:
        ...
        if message_size > self.settings.max_frame_bytes:
            raise ValueError(
                ...
                f"{message_size} bytes, exceeding TRACEFANG_NATS_MAX_FRAME_BYTES="
    ...
    async def _ensure_stream(self, jetstream: JetStreamContext) -> None:
        ...
        if not stream_can_carry_limit and info.state.messages != 0:
            raise RuntimeError(
                ...
                f"TRACEFANG_NATS_MAX_FRAME_BYTES={self.settings.max_frame_bytes}; "
                "preserve the recorded evidence and select a new "
                "TRACEFANG_NATS_STREAM, or empty/recreate the stream with a "
