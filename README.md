...
- 每个合约独立保存一个完整行情源；报价、K 线、分时、状态与延迟始终来自同一来源；
- 一次请求或订阅只绑定一个明确来源，断线时标记停滞，不自动换源；
...
XAUUSD 可以使用本地结构化源，同时 XAGUSD 使用官方 MCP。来源失效只影响绑定它的合约，不会把另一来源的数据拼入当前序列。本地结构化研究路线见 [docs/local-data-acceleration.md](docs/local-data-acceleration.md)。
...
- 路由粒度是“合约”。切换合约时恢复该合约自己的完整行情源。
- 所有来源选择都是强制来源，失败时直接报告，不暗中换源。
- `连接并测试`：限额或付费来源默认不连接；用户主动点击后才完成协议握手并测试报价。测试不会自动切换当前合约，测试官方源会明确消耗一次官方报价调用。
- K 线只从当前合约所选来源读取：官方 MCP 使用 `get_kline`；本地源用自身结构化报价流生成并持久化分钟 K 线。历史不足时只向同一来源补齐，不借用其他来源。
- 当前柱在浏览器中用“同源最近一分钟 K 线 + 同源当前报价”合成：同一周期内只更新真实报价触达后的收盘/最高/最低，跨周期后由下一笔报价开启新柱。倒计时每秒更新，不额外调用数据接口。
...
合约的唯一行情源保存在 PostgreSQL 的 `instrument_source_routes` 兼容表中；应用会把其中报价与 K 线两行原子写成同一来源。来源启停和页面顺序保存在 `data/sources.json`。接口契约见 [docs/source-management.md](docs/source-management.md)。
